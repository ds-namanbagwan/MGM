...
const metersToMiles = (meters: number) => {
  ...
  let finalmiles:any = miles.toFixed(2);
...
const LocationCard: CardComponent<Location> = (props: any) => {
  ...
  function opentime(e: any) {
    ...
    if (
      ...
      closethis
        ...
        closethis.querySelector(".open-now-string").classList.add('active')
      setHoursopen(true);
    } else {
      closethis
        .querySelector(".storelocation-openCloseTime")
        .classList.add("hidden");
        closethis.querySelector(".open-now-string").classList.remove('active')
      setHoursopen(false);
    }
    if (timeStatus == "") {
  ...
  return props.result.map((result: any, keyindex: number) => {
    ...
    const finalslug:any=constant.slugify(result1);
    if (!result.rawData.slug) {
      url =
        props.mode === "development"
          ? `/location/${result.rawData.id}?locale=en_GB`
          : `${constant.stagingBaseurl}/${constant.slugify(result.rawData.id)}-${finalslug}`;
    } else {
      url =
        props.mode === "development"
          ? `/location/${result.rawData.slug}?locale=en_GB`
          : `${constant.stagingBaseurl}/${constant.slugify(result.rawData.slug)}`;
    }
            ...
            <div className="lp-param-results lp-subparam-hours">
              <div className="location-name-miles icon-row">
                ...
                <div className="distance">
                  <img src={map_marker} alt="" height={29} width={23} />
              ...
              <div className="icon-row content-col address-with-availablity">
                ...
                {result.rawData.mainPhone && (
                  <div className="address notHighlight items-center">
                    ...
                    <Link
                      ...
                      eventName="phone"
                      className="underline"
                      href={`tel:${result.rawData.mainPhone.replace("+44","0")}`}
                      rel="noopener noreferrer"
                    >
                      {result.rawData.mainPhone.replace("+44","0")}
                      </Link>
                    {/* <a href={`tel:${result.rawData.mainPhone.replace("+44","0")}`}>{result.rawData.mainPhone.replace("+44","0")}</a> */}
                  </div>
                )}
                ...
                {result.rawData.emails && (
                  <div className="address notHighlight items-center">
                    ...
                    <a href={`mailto:${result.rawData.emails[0]}`}>
                ...
                <div className="mt-2">
                  {result.rawData.hours?.reopenDate ? (
                    <>
                      ...
                      <div
                        className="open-now-string cursor-pointer flex  items-center "
                    ...
                    <>
                      <div
                        className={"open-now-string notHighlight flex  items-center cursor-pointer"}
                        data-id={`main-shop-${result.rawData.id}`}
                        onClick={opentime}
                      >
                        <OpenClose
              ...
              <div className="button-bx">
                ...
                <Link
                  ...
                  rel="noopener noreferrer"
                >
                  <img src={storedetail} alt="" height={20} width={21} />
